...
- **Server adapter** (`better-blog/server`)
  - `createBlogServerAdapter({ provider: BlogDataProvider, queryClient: QueryClient })` → `{ generateStaticParams, generateMetadata, BlogServerRouter }`
  - `createBlogApiRouter({ provider: BlogDataProvider, basePath?: string, openapi?: ... })` → `{ router }`
