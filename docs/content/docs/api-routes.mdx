...
```tsx
import { BlogProvider, useBlogContext } from "better-blog/client";
import { createBlogApiProvider } from "better-blog/core/providers/api-provider";

// Provide API-backed client with a provider-wide default locale
<BlogProvider
  dataProvider={createBlogApiProvider({ basePath: "/api/posts", defaultLocale: "en" })}
>
  {children}
</BlogProvider>
```
