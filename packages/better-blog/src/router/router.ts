...
export function matchRoute(pathSegments?: string[], basePath?: string): RouteMatch {
  ...
  if (basePath && normalizedSlug[0] === basePath.split('/').filter(Boolean)[0]) {
  ...
  for (let index = 0; index < routeSchema.routes.length; index++) {
    ...
    const specificityScore = routeDef.pattern.reduce((count, segment) => (
      segment.startsWith(':') ? count : count + 1
    ), 0);
