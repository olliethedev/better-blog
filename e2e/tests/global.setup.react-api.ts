...
setup('write env and start react (api)', async () => {
  const envPath = join('.e2e', `${PROJECT}.env`);
  await mkdir(dirname(envPath), { recursive: true });
  const contents = [
    'E2E=1',
    'BETTER_BLOG_PROVIDER=api',
    'API_BASE_PATH=/api/posts'
  ].join('\n');
  await writeFile(envPath, contents, 'utf8');

  // biome-ignore lint/style/noUnusedTemplateLiteral: <explanation>
  const proc = spawn('node', ['e2e/scripts/run-webserver.mjs', `--framework=react`, `--project=${PROJECT}`, `--port=${PORT}`, `--script=start:node`], {
    stdio: 'ignore',
    detached: true,
    env: process.env,
  });
