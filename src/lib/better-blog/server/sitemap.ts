...
export interface CreateBlogSitemapConfig {
...
function joinPaths(...segments: string[]): string {
    ...
    return joined.startsWith("http") ? joined : `/${joined}`.replace(/\/+/g, "/")
}
...
export async function createBlogSitemap(config: CreateBlogSitemapConfig): Promise<Sitemap> {
    ...
    const posts = (await config.data.getAllPosts({ published: true })) ?? []
    ...
    const tagSlugs = Array.from(
        new Set(
            posts.flatMap((p) => p.tags?.map((t) => t.slug).filter(Boolean) ?? [])
        )
    )
